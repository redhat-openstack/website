...
the CloudSIG using [rdopkg](/documentation/intro-packaging/#rdopkg):
    ...
    - phase:
      ...
      - `el9s-build` is assigned to packages that only required to build other
...
Two patches will be triggered automatically based on last review:
    1. The first one will define proper resources (group, ACL, repo) for the project within RDO config repo.
       Once merged, the repository will be created under ‘https://review.rdoproject.org/r/deps/<name>.git’
       For example: https://review.rdoproject.org/r/c/config/+/36059
    2. The second one will add the project to the RDO Continuous Integration system.
       Note: this patch is pushed on top of the first one because resources must be declared first.
       Patch example: https://review.rdoproject.org/r/c/config/+/36060

Once those 2 patches are merged, you need to propose a review to build it.
We have a tool which does the work: [setup_distgit.sh](https://review.rdoproject.org/cgit/gating_scripts/tree/setup_distgit.sh)
        ...
        $ cd gating_scripts
        $ bash -x setup_distgit.sh <project_name> <build> yoga 9s
