...
2. Create the virtual-power-key and copy it to the host. The user in ssh-copy-id should match the user you created on the virt **host** earlier. The IP in ssh-copy-id should be the IP of the virt **host** machine on the VM network, typically 192.168.122.1. Make a note of these two values as they will be the VIRTUAL_POWER_USER and VIRTUAL_POWER_HOST settings respectively in the instack.answers file discussed later.
